...
<<<<<<< HEAD
                ...
                if len(requests) - num_decode_req > 0:
                    intermed.append(self.prefill_wrapper.run(
                        q[num_decode_req:], 
                        (self.pool.k_datas[layer], self.pool.v_datas[layer])
                    ))
...
<<<<<<< HEAD
            ...
            rms = torch.sqrt(hidden_last.square().mean(-1, keepdim=True) + 1e-5)
            logits = (
                (hidden_last / rms).to(torch.float16) * self.weights["model_layernorm_weight"]
