...
<<<<<<< HEAD
        ...
        q = x.matmul(self_attn_q_proj_weight[layer][left_q : right_q].t()) # (seq_len, hidden_dim): (seq_len, 4096)

        # For k, v, num_kv_heads = 8, so w is (num_kv_heads * head_dim, hidden_dim): (8 * 128, 4096)
        left_kv = rank * local_kv_heads * head_dim
        right_kv = left_kv + local_kv_heads * head_dim
...
<<<<<<< HEAD
        ...
        o_proj_weight_local = o_proj_weight[layer].t()[left_o:right_o]
        o_proj_local = attn_output.matmul(o_proj_weight_local)
...
<<<<<<< HEAD
        ...
        right_up = left_up + local_intermediate_dim
        up_proj_weight_local = up_proj_weight[layer][left_up : right_up].t()
        up_local = ffn_input.matmul(up_proj_weight_local)

        left_gate = left_up
        right_gate = right_up
        gate_proj_weight_local = gate_proj_weight[layer][left_gate : right_gate].t()
        gate_local = ffn_input.matmul(gate_proj_weight_local)
        ...
        left_down = left_up
        right_down = right_up
        down_proj_weight_local = down_proj_weight[layer].t()[left_down:right_down]
        down_local = activation_output.matmul(down_proj_weight_local)
