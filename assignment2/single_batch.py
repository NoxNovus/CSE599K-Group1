    ...
    def run(self, input_ids, prefill = True):
        ...
        for current_layer in range(self.layers):
            ...
            if (prefill):
                ...
                pos = self.kv_cache[current_layer]["k"].shape[0]
                apply_rope(q, output=q, head_dim=self.head_dim, offset=pos)
                apply_rope(k, output=k, head_dim=self.head_dim, offset=pos)
            ...
            if (prefill):
                ...
                sub_k = torch.cat([self.kv_cache[current_layer]["k"], sub_k], axis = 0)
                sub_v = torch.cat([self.kv_cache[current_layer]["v"], sub_v], axis = 0)
